...
ECHO 
ECHO 'Output results will be saved to '$red'serverles-apim-setup.log'$reset' file ...'
ECHO
ECHO '- Creating '$red$resourceGroup$reset' Resource Group ...'
az group create -n $resourceGroup -l $location --output json > serverles-apim-setup.log

ECHO '- Creating '$red$storageAccountName$reset' Storage Account ...' 
az storage account create -n $storageAccountName -l $location -g $resourceGroup --sku Standard_LRS --output json >> serverles-apim-setup.log
...
az functionapp create -n $productsFunctionName --storage-account $storageAccountName --consumption-plan-location $location --app-insights $appInsightsName --runtime dotnet -g $resourceGroup  --functions-version 3  >> serverles-apim-setup.log

ECHO '- Creating '$red$reviewsFunctionName$reset' Function App ...'
az functionapp create -n $reviewsFunctionName --storage-account $storageAccountName --consumption-plan-location $location --app-insights $appInsightsName --runtime dotnet -g $resourceGroup  --functions-version 3 --runtime node >> serverles-apim-setup.log

ECHO '- Creating '$red'serverless-api'$rand$reset' API Management [Consuption Plan] ...'
az apim create --name serverless-apis$rand -g $resourceGroup -l eastus --publisher-email myemail@mycompany.com --publisher-name microsoft --sku-name consumption -o json >> serverles-apim-setup.log

ECHO '- Deploying '$red$productsFunctionName$reset' to '$red$productsFunctionName$reset' Function App ...'
cd apis/Products
func azure functionapp publish  $productsFunctionName --output json >> ../../serverles-apim-setup.log

ECHO '- Deploying '$red$reviewsFunctionName$reset' to '$red$reviewsFunctionName$reset' Function App ...'
cd ../Reviews
func azure functionapp publish $reviewsFunctionName --typescript --output json >> ../../serverles-apim-setup.log
cd ../..
